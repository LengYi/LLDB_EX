...
def get_Address(command):
	...
	interpreter = lldb.debugger.GetCommandInterpreter()
	returnObject = lldb.SBCommandReturnObject()
	interpreter.HandleCommand('memory read --size 4 --format x %s' % command,returnObject)
	output = returnObject.GetOutput();
...
def block(debugger,command,result,internal_dict):
	#用户是否输入了地址参数
	if not command:
	...
	# block函数描述结构体地址连续地址
	blocDesAddressList = get_Address(blockAddressList[6])
	# char 类型参数
	blockParamsCharName = get_block_funcDesChar(blocDesAddressList[6])
	# block 函数参数
...
def __lldb_init_module(debugger,internal_dict):
	# 'command script add block' : 给lldb增加一个'block'命令
	# '-f block.block' : 该命令调用了block文件的block函数	 
	debugger.HandleCommand('command script add block -f block.block')
	print 'The "block" python command has been installed and is ready for use.'